...
- **Verified:** 2026-09-01
- **Inputs:** `image` — or the base named by `build.dockerfile` — and `features` with each entry's options; the authored Feature's `install.sh`, for the idempotency check.
...
- **Verified:** 2026-09-01
- **Inputs:** every lifecycle hook — `initializeCommand`, `onCreateCommand`, `updateContentCommand`, `postCreateCommand`, `postStartCommand` and `postAttachCommand` — and which of the three forms each one uses; the image metadata label when any of those hooks is absent from the file.
